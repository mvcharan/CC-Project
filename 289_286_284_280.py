...
#         container.remove()
#         del nodes[node_id]
#         return jsonify({"message": "Node removed", "node_id": node_id})
#     except Exception as e:
#         return jsonify({"error": str(e)}), 500
...
def stop_node(node_id):
    ...
    try:
        ...
        # Remove all associated pods
        orphaned_pods = [pod_id for pod_id, pod in pods.items() if pod['node_id'] == node_id]
        for pod_id in orphaned_pods:
            del pods[pod_id]

        del nodes[node_id]
        return jsonify({"message": "Node and its pods removed", "node_id": node_id, "removed_pods": orphaned_pods})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
